...
namespace FishGame
{
    ...
    PoisonFish::PoisonFish()
        : Hazard(HazardType::PoisonFish, 0.0f)
        , m_shape(20.0f)
        , m_poisonBubbles()
        ...
        , m_poisonDuration(sf::seconds(m_poisonEffectDuration))
    {
        ...
        m_shape.setOutlineColor(sf::Color(100, 0, 150));
        m_shape.setOutlineThickness(2.0f);
        m_shape.setOrigin(m_radius, m_radius);

        // Create poison bubble effects
        m_poisonBubbles.reserve(6);
        for (int i = 0; i < 6; ++i)
        {
            sf::CircleShape bubble(3.0f);
            bubble.setFillColor(sf::Color(200, 100, 255, 150));
            bubble.setOrigin(3.0f, 3.0f);
    ...
    void PoisonFish::update(sf::Time deltaTime)
    {
        ...
        float wobble = std::sin(m_wobbleAnimation) * 5.0f;

        m_shape.setPosition(m_position.x, m_position.y + wobble);

        // Update poison bubbles
        for (size_t i = 0; i < m_poisonBubbles.size(); ++i)
        {
            float angle = (60.0f * i + m_wobbleAnimation * 30.0f) * 3.14159f / 180.0f;
            float radius = 25.0f + 5.0f * std::sin(m_wobbleAnimation + i);

            sf::Vector2f bubblePos(
                m_position.x + std::cos(angle) * radius,
                m_position.y + std::sin(angle) * radius + wobble
            );
        ...
        // Check boundaries
        if ((m_velocity.x > 0 && m_position.x > 1920.0f + m_radius) ||
            (m_velocity.x < 0 && m_position.x < -m_radius))
        {
            destroy();
        }
    }

    sf::FloatRect PoisonFish::getBounds() const
    {
        return sf::FloatRect(m_position.x - m_radius, m_position.y - m_radius,
            m_radius * 2.0f, m_radius * 2.0f);
    }

    void PoisonFish::onContact(Entity& entity)
    {
        if (entity.getType() == EntityType::Player)
        {
            // Effect handled by PlayState
            destroy();
        }
    }
